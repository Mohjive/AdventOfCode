...
typedef unordered_set<int> overlapped_set;

pair<fabric::Fabric,overlapped_set> weave_fabric( fabric::Claims const &claims )
{
    fabric::Fabric fabric;
    overlapped_set overlapped;
    for ( auto const &claim: claims )
    {
        for ( int x = claim.coord.x;
              x < claim.coord.x + claim.size.width;
              x++ )
        {
            for ( int y = claim.coord.y;
                  y < claim.coord.y + claim.size.height;
                  y++ )
            {
...
int second( unordered_set<int> const &overlapped,  fabric::Claims const &claims) {
    for (auto const &claim: claims)
    {
        if (overlapped.find(claim.id) == overlapped.end()) return claim.id;
    }
    return 0;
}
...
int main()
{
    ...
    {
        if (s1 != '#' && s2 != '@' && s3 != ',' && s4 != ':' && s5 != 'x') std::terminate();
        claims.push_back(c);
    }
